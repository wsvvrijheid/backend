...
const trends = require('./functions/trends')
...
module.exports = strapi => {
  const hook = {
    ...
    async initialize() {
      ...
      if (
        !settings.consumer_key.length ||
        !settings.consumer_secret.length ||
        !settings.access_token_key.length ||
        !settings.access_token_secret.length
      ) {
      ...
      const errorHandler = e => {
        if ('errors' in e) {
          if (e.errors[0].code === 88)
            console.error(
              'twitter hook error: rate limit will reset on',
      ...
      const savedSearchFunch = saved_search.savedSearch(
        client,
        errorHandler,
        settings.debug,
      )
      const timelineFunc = timeline.timeline(
        client,
        errorHandler,
        settings.debug,
      )
      ...
      const trendsFunc = trends.trends(client, errorHandler, settings.debug)
      ...
      const executeFunc = async (api, method, parameters) => {
        try {
          ...
          if (response) {
            if (settings.debug)
              console.log(
                `twitter hook - execute ${method} '${api}': `,
                response,
              )
